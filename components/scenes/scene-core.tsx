...
export function SceneCore() {
  ...
  const phoneScale = 0.6 + animProgress * 0.4
  const phoneOpacity = Math.min(animProgress * 2, 1)
    ...
    <section ref={sectionRef} className="relative" style={{ height: "300vh" }}>
      <div className="sticky top-0 flex h-screen items-center justify-center overflow-hidden">
        ...
        <div
          ...
          style={{
            opacity: Math.min(animProgress * 2.5, 1),
            transform: `translateY(${(1 - Math.min(animProgress * 2.5, 1)) * 30}px)`,
          }}
        >
        ...
        {/* Features row */}
        <div
          className="absolute bottom-[8%] z-20 flex flex-col items-center gap-4 px-4 md:bottom-[10%] md:flex-row md:gap-10"
          style={{
            opacity: Math.min(Math.max((animProgress - 0.5) / 0.3, 0), 1),
            transform: `translateY(${(1 - Math.min(Math.max((animProgress - 0.5) / 0.3, 0), 1)) * 20}px)`,
          }}
        >
          {features.map((feat, i) => {
            ...
            return (
              <div
                key={i}
                className="flex items-center gap-3 rounded-xl border border-[#F8F5F2]/10 bg-[#0A261D]/60 px-5 py-3 backdrop-blur-sm"
              >
